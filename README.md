...
本サイトは Jekyll を使用して構築されています。また、表示されるデータセット情報の多くはスクリプトを使用して事前に生成されます。ローカルで動作確認・開発を行いたい場合は、以下のコマンドを実行してください。
...
（※注意: `_config.yml` 等の根幹設定ファイルを書き換えた場合は、変更を反映させるために `jekyll serve` を一度停止（`Ctrl+C`）し、再実行する必要があります）
...
#### 4-1. サーバー上での手動ビルド運用について
GitHub Actions を経由せず、サーバー上で直接リポジトリを `git pull` して手動でビルド・デプロイを行う場合は、**開発用の `jekyll serve` を本番環境で起動しっぱなしにすることは避けてください**（キャッシュの同期不整合や、設定ファイルの変更が反映されないバグの原因となります）。
...
```
（完了後、生成された `_site` フォルダの中身を、Nginx等のドキュメントルートに配置・展開してください。）

...
class DatasetCard {
  ...
  #buildPetalSvg() {
    ...
    if (this.#dataset && this.#dataset.id) {
      return `<img src="${baseUrl}/assets/images/dataset_symbols/${this.#escapeHtml(this.#dataset.id)}.svg" class="dataset-icon" width="${size}" height="${size}" alt="Icon" decoding="sync" loading="eager" onerror="this.outerHTML='<svg class=\\'dataset-icon\\' width=\\'${size}\\' height=\\'${size}\\' viewBox=\\'0 0 ${size} ${size}\\'><rect width=\\'${size}\\' height=\\'${size}\\' fill=\\'#ddd\\'/></svg>'"/>`;
    }
